...
let calculateIntersection = (aDirections, bDirections) => {
  ...
  function executeJourney (wire) {
    ...
    for(let i = 0; i < wire.length; i++) {
      ...
      if(direction == "R") {
        for(let j = currentCoords[0]; j < (currentCoords[0] + parseInt(wire[i].slice(1))); j++ ) {
          let coords = [j, currentCoords[1]]
          map1.set(coords, [i+j]);
        }
        currentCoords[0] = currentCoords[0] + parseInt(wire[i].slice(1));
      }
      if(direction == "L") {
        for(let j = currentCoords[0]; j > (currentCoords[0] - parseInt(wire[i].slice(1))); j-- ) {
          let coords = `${j}, ${currentCoords[1]}`
      ...
      if(direction == "U") {
        for(let j = currentCoords[1]; j < (currentCoords[1] + parseInt(wire[i].slice(1))); j++ ) {
          let coords = `${currentCoords[0]}, ${j}`
          map1.set(coords,`${i}+${j}`);

        }
        currentCoords[1] = currentCoords[1] + parseInt(wire[i].slice(1));
      }
  ...
  function findMatches (a, b) {
    ...
    let matches = []
    // let shortest;
    // if(a.length <= b.length) { shortest = a;}
    // if(b.length < a.length) { shortest = b;}


    function logMapElements(value, key, map) {

   if(b.has(key)) {matches.push([key.to_i])}
  // console.log(value);
  // console.log(key);
  // console.log(map);
  // console.log("----");
}
a.forEach(logMapElements);
// console.log(matches);

  //
    // for (var i = 0, keys = Object.keys(a), ii = keys.length; i < ii; i++) {
    //   console.log(i);
    //   console.log(a[i]);
    //
    // }
  //
  // let valueToCompare = a[keys[i]];
  // console.log(valueToCompare);

// Object.keys(b).forEach(function(key) {
//
//  if (b[key] == valueToCompare) {
//    console.log("MATCHHH");
//    matches[i] = b[key]
//  }
// });
...
// }
 // console.log(matches)
  ...
  function findMinDistance(matches) {
    console.log(matches);
    let distances = []
    for(let i=0; i < matches.length; i++) {
      distances.push(Math.abs(matches[i][0]) + Math.abs(matches[i][1]))
    }
    console.log(Math.min(...distances));

  }

  let aPoints = executeJourney(formatDirections(aDirections));
  let bPoints = executeJourney(formatDirections(bDirections));
  //
   let matches = findMatches(aPoints, bPoints)
 //  //
  // if(matches === undefined || matches.length == 0) {
  //   console.log("No matches");
  // }
  // else {
  //   findMinDistance(findMatches(aPoints, bPoints));
  // }
