...
let calculateIntersection = (aDirections, bDirections) => {
  ...
  let matches = findMinDistance(findMatches(aPoints, bPoints))
  ...
  function findMatches (a, b) {
    ...
    function logMapElements(value, key, map) {
      ...
      if(b.has(key)) {
        matches.push([key])
        squareDistance.push([key, b.get(key)])
  ...
  function findMinDistance(matches) {
    ...
    for(let i=0; i < matches.length; i++) {
      ...
      let m = Math.abs(matches[i][0].split(",")[0])
      let y = Math.abs(matches[i][0].split(",")[1].trim()) // remove whitespace

      if(!(m==0 && y == 0)){
        distancesResult.push(m+y)
      }

    }
    // console.log(matches);
    // console.log(Math.min(...distancesResult))
  }
}
